...
namespace MoreMountains.InfiniteRunnerEngine
{
	...
	public class DistanceSpawner : Spawner
	{
		...
		protected virtual void DistanceSpawn(Vector3 spawnPosition)
		{
			...
			if (_lastSpawnedTransform != null)
			{
				...
				spawnedObject.transform.position += transform.rotation
				                                    ...
				                                    * (xDistanceToLastSpawnedObject
				                                       + _lastSpawnedTransform.GetComponent<MMPoolableObject>().Size.x /
				                                       2
				...
				{
					spawnedObject.transform.position += (transform.rotation *
					                                     ClampedPosition(MMMaths.RandomVector3(MinimumGap,
						                                     MaximumGap)));
				}
				else
				{
					...
					spawnedObject.transform.position = (transform.rotation *
					                                    ClampedPositionRelative(spawnedObject.transform.position,
						                                    transform.position));
				}
			}
			else
			{
				...
				spawnedObject.transform.position +=
					(transform.rotation * ClampedPosition(MMMaths.RandomVector3(MinimumGap, MaximumGap)));
			}
		...
		{
			vectorToClamp.y =
				Mathf.Clamp(vectorToClamp.y, MinimumYClamp + clampOrigin.y, MaximumYClamp + clampOrigin.y);
			vectorToClamp.z =
				Mathf.Clamp(vectorToClamp.z, MinimumZClamp + clampOrigin.z, MaximumZClamp + clampOrigin.z);
			return vectorToClamp;
		}
		...
		protected virtual void OnDrawGizmosSelected()
		{
			...
			if (MinimumGap != Vector3.zero)
			{
				...
				Vector3 labelPosition =
					transform.position + (Mathf.Abs(MinimumGap.y / 2) + 1) * Vector3.up + Vector3.left;
				labelPosition =
					MMMaths.RotatePointAroundPivot(labelPosition, transform.position, transform.rotation.eulerAngles);
				#if UNITY_EDITOR
			...
			if (MaximumGap != Vector3.zero)
			{
				...
				Vector3 labelPosition =
					transform.position + (-Mathf.Abs(MaximumGap.y / 2) + 1) * Vector3.up + Vector3.left;
				labelPosition =
					MMMaths.RotatePointAroundPivot(labelPosition, transform.position, transform.rotation.eulerAngles);
				#if UNITY_EDITOR
		...
		protected virtual void DrawClamps()
		{
			...
			Gizmos.DrawLine(
				MMMaths.RotatePointAroundPivot(new Vector3(xMinus5, minimumYClamp, minimumZClamp), transform.position,
					transform.rotation.eulerAngles),
				MMMaths.RotatePointAroundPivot(new Vector3(xPlus5, minimumYClamp, minimumZClamp), transform.position,
					transform.rotation.eulerAngles));

			Gizmos.DrawLine(
				MMMaths.RotatePointAroundPivot(new Vector3(xMinus5, maximumYClamp, minimumZClamp), transform.position,
					transform.rotation.eulerAngles),
				MMMaths.RotatePointAroundPivot(new Vector3(xPlus5, maximumYClamp, minimumZClamp), transform.position,
					transform.rotation.eulerAngles));

			Gizmos.DrawLine(
				MMMaths.RotatePointAroundPivot(new Vector3(xMinus5, minimumYClamp, maximumZClamp), transform.position,
					transform.rotation.eulerAngles),
				MMMaths.RotatePointAroundPivot(new Vector3(xPlus5, minimumYClamp, maximumZClamp), transform.position,
					transform.rotation.eulerAngles));

			Gizmos.DrawLine(
				MMMaths.RotatePointAroundPivot(new Vector3(xMinus5, maximumYClamp, maximumZClamp), transform.position,
					transform.rotation.eulerAngles),
				MMMaths.RotatePointAroundPivot(new Vector3(xPlus5, maximumYClamp, maximumZClamp), transform.position,
					transform.rotation.eulerAngles));
