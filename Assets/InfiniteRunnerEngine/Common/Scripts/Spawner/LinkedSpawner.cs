...
namespace MoreMountains.InfiniteRunnerEngine
{
	...
	public class LinkedSpawner : Spawner
	{
		...
		[Tooltip("the x distance we spawn the player at")]
		public float SpawnDistanceFromPlayer;

		[Space(10)] [Header("Random Spacing")] public Vector3 MinimumSpacing = Vector3.zero;
				...
				if (transform.position.x - _lastSpawnedTransform.position.x >= _nextSpawnDistance)
				{
					...
					if (_lastSpawnedTransform != this.transform)
					{
						spawnPosition = _lastSpawnedTransform.transform.position +
						                _lastSpawnedTransform.GetComponent<LinkedSpawnedObject>().Out;
					}
					else
					{
						spawnPosition = _lastSpawnedTransform.transform.position;
		...
		protected virtual void LinkedSpawn(Vector3 spawnPosition)
		{
			...
			else
			{
				...
				if (_lastLinkedSpawnedObject != null)
				{
					...
					spawnedObject.transform.position = _lastSpawnedTransform.position + _lastLinkedSpawnedObject.Out -
					                                   spawnedLinkedSpawnedObject.In;
