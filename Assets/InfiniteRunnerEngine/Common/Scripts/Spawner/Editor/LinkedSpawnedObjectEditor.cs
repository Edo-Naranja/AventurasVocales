...
public class LinkedSpawnedObjectEditor : Editor
{
	...
	public virtual void OnSceneGUI()
	{
		...
		DrawHandle(_linkedSpawnedObject.gameObject, _linkedSpawnedObject.In, Color.green,
			_linkedSpawnedObject.gameObject.name + " In");
		DrawHandle(_linkedSpawnedObject.gameObject, _linkedSpawnedObject.Out, Color.red,
			_linkedSpawnedObject.gameObject.name + " Out");
	}

	protected virtual void DrawHandle(GameObject handleObject, Vector3 handlePosition, Color handleColor,
		String handleName)
	{
	...
	{
		_linkedSpawnedObject.In =
			_linkedSpawnedObject.transform.InverseTransformPoint(_linkedSpawnedObject.GetComponent<Renderer>().bounds
				.min);
		_linkedSpawnedObject.Out =
			_linkedSpawnedObject.transform.InverseTransformPoint(_linkedSpawnedObject.GetComponent<Renderer>().bounds
				.max);
