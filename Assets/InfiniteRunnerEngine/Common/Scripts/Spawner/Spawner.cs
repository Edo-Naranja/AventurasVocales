...
namespace MoreMountains.InfiniteRunnerEngine
{
	...
	public class Spawner : MonoBehaviour
	{
		...
		[Space(10)] [Header("When can it spawn?")]
		/// if true, the spawner can spawn, if not, it won't spawn
		[Tooltip("if true, the spawner can spawn, if not, it won't spawn")]
		public bool Spawning = true;

		/// if true, only spawn objects while the game is in progress
		[Tooltip("if true, only spawn objects while the game is in progress")]
		public bool OnlySpawnWhileGameInProgress = true;

		/// Initial delay before the first spawn, in seconds.
		[Tooltip("Initial delay before the first spawn, in seconds.")]
		...
		public virtual GameObject Spawn(Vector3 spawnPosition, bool triggerObjectActivation = true)
		{
			...
			if (!PreserveRatio)
			{
				newScale = new Vector3(UnityEngine.Random.Range(MinimumSize.x, MaximumSize.x),
					UnityEngine.Random.Range(MinimumSize.y, MaximumSize.y),
					UnityEngine.Random.Range(MinimumSize.z, MaximumSize.z));
			}
			else
			{
