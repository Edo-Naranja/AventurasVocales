...
namespace MoreMountains.InfiniteRunnerEngine
{
	...
	public class CameraBehavior : MonoBehaviour
	{
		...
		[Tooltip("the minimum possible position for the camera")]
		public Vector3 MinimumZoom;

		/// the minimum ortho zoom
		[Tooltip("the minimum ortho zoom")] public float MinimumZoomOrthographic = 3f;

		[Tooltip("the maximum possible position for the camera")]
		/// the maximum possible position for the camera
		public Vector3 MaximumZoom;

		/// the maximum ortho zoom
		[Tooltip("the maximum ortho zoom")] public float MaximumZoomOrthographic = 5f;
		...
		[Tooltip("true if the camera should follow the player on the y axis")]
		public bool FollowsPlayerY = true;

		/// true if the camera should follow the player on the z axis
		[Tooltip("true if the camera should follow the player on the z axis")]
		...
		[Tooltip("if true, the camera won't be able to escape its bounding box")]
		public bool CameraCanOnlyMoveWithinBounds = true;

		/// the position of the bottom left corner of the camera's bounding box
		[Tooltip("the position of the bottom left corner of the camera's bounding box")]
		public Vector2 BottomLeftBounds;

		/// the position of the top right corner of the camera's bounding box
		[Tooltip("the position of the top right corner of the camera's bounding box")]
		public Vector2 TopRightBounds;

		[Header("Target Override")]
		[Tooltip("If you set this, the camera will follow this target instead of the player")]
		...
		protected virtual void Update()
		{
			...
			_camera.orthographicSize = MinimumZoomOrthographic + (MaximumZoomOrthographic - MinimumZoomOrthographic) *
				(LevelManager.Instance.Speed / LevelManager.Instance.MaximumSpeed);
			_currentZoomOffset = MinimumZoom + (MaximumZoom - MinimumZoom) *
				(LevelManager.Instance.Speed / LevelManager.Instance.MaximumSpeed);

			// we get the bounds 
			GetLevelBounds();
			// we interpolate the camera's position towards the new player's position, taking into account the maximum zoom.
			_newCameraPosition = Vector3.Lerp(transform.position, _currentZoomOffset + _playerPosition,
