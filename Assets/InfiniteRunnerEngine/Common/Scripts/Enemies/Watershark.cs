...
namespace MoreMountains.InfiniteRunnerEngine
{
	...
	public class Watershark : MonoBehaviour
	{
		...
		[Tooltip("the explosion gameobject that we'll make appear when the shark collides with the player")]
		...
		protected virtual void OnTriggerEnter2D(Collider2D collidingObject)
		{
			...
			explosion.transform.position = new Vector3(transform.GetComponent<Renderer>().bounds.min.x,
				transform.GetComponent<Renderer>().bounds.center.y, 0);
