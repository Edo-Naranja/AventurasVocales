	...
	public class GUIManager : MMSingleton<GUIManager>, MMEventListener<MMGameEvent>
	{
		...
		public virtual void InitializeLives()
		{
			...
			for (int i = 0; i < GameManager.Instance.TotalLives; i++)
			{
				...
				heart.GetComponent<RectTransform>().localPosition = new Vector3(
					HeartsContainer.GetComponent<RectTransform>().sizeDelta.x / 2 -
					i * (heart.GetComponent<RectTransform>().sizeDelta.x * 75f), 0, 0);
				deadLives--;
			}
		}
