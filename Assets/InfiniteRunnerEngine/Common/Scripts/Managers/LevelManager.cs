...
namespace MoreMountains.InfiniteRunnerEngine
{
	...
	public class LevelManager : MMSingleton<LevelManager>
	{
		...
		[Tooltip("the list of playable characters - use this to tell what characters you want in your level, don't access that at runtime")]
		...
		[Tooltip("the list of playable characters currently instantiated in the game - use this to know what characters ARE currently in your level at runtime")]
		public List<PlayableCharacter> CurrentPlayableCharacters { get; set; }

		/// the x distance between each character
		[Tooltip("the x distance between each character")]
		...
		[Tooltip("the amount of points a player gets per second")]
		public float PointsPerSecond = 20;

		/// the text that will be shown (if not empty) at the start of the level
		[Multiline] 
		[Tooltip("the text that will be shown (if not empty) at the start of the level")]
		public String InstructionsText;

		[Space(10)] [Header("Level Bounds")]
		/// the line after which objects can be recycled
		[Tooltip("the line after which objects can be recycled")]
		...
		[Tooltip("the line after which playable characters will die - leave it to zero if you don't want to use it")]
		...
		[Tooltip("the maximum speed the level will run at")]
		public float MaximumSpeed = 50f;

		/// the acceleration (per second) at which the level will go from InitialSpeed to MaximumSpeed
		[Tooltip("the acceleration (per second) at which the level will go from InitialSpeed to MaximumSpeed")]
		public float SpeedAcceleration = 1f;

		[Space(10)] [Header("Intro and Outro durations")]
		...
		[Tooltip("duration of the fade to black at the end of the level")]
		public float OutroFadeDuration = 1f;

		[Space(10)] [Header("Start")]
		/// the duration (in seconds) of the initial countdown
		[Tooltip("the duration (in seconds) of the initial countdown")]
		public int StartCountdown;

		/// the text displayed at the end of the countdown
		[Tooltip("the text displayed at the end of the countdown")]
		public string StartText;

		[Space(10)] [Header("Mobile Controls")]
		/// the mobile control scheme applied to this level
		[Tooltip("the mobile control scheme applied to this level")]
		public Controls ControlScheme;

		[Space(10)] [Header("Life Lost")]
		...
		protected virtual void InstantiateCharacters()
		{
			...
			{
				PlayableCharacter newPlayer = (PlayableCharacter)Instantiate(
					CharacterSelectorManager.Instance.StoredCharacter, StartingPosition.transform.position,
				...
				MMGameEvent.Trigger("PlayableCharactersInstantiated");
				return;
			}
		...
		protected virtual void ManageControlScheme()
		{
			...
			switch (ControlScheme)
			{
				case Controls.SingleButton:
					...
					if (GUIManager.Instance.transform.Find(buttonPath) == null)
					{
						return;
					}
