...
namespace MoreMountains.InfiniteRunnerEngine
{
	...
	public class ButtonNavigation : MonoBehaviour
	{
		...
		[Tooltip("the list of buttons the player will be able to navigate through")]
		...
		[Tooltip("the axis to use for navigation")]
		public AxisChoices Axis;
		/// if set to true, the first button will be automatically selected
		[Tooltip("if set to true, the first button will be automatically selected")]
		...
		protected virtual void OnGUI()
		{
			...
			if (AutoPressButtonOnFocus && _buttonPressRequired)
			{
				...
				ExecuteEvents.Execute(ButtonList[_selectedIndex].gameObject, tempEventData,
