...
namespace MoreMountains.InfiniteRunnerEngine
{
	...
	public class RepositionOnStart2D : MonoBehaviour
	{
		...
		{
			RaycastHit2D raycastUpwards = MMDebug.RayCast(transform.position, Vector2.up, RaycastLength,
				1 << LayerMask.NameToLayer("Ground"), Color.gray, true);
			RaycastHit2D raycastDownwards = MMDebug.RayCast(transform.position, Vector2.up, RaycastLength,
				1 << LayerMask.NameToLayer("Ground"), Color.gray, true);
