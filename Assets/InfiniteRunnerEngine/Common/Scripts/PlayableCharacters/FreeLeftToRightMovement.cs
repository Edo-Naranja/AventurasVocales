...
namespace MoreMountains.InfiniteRunnerEngine
{
	...
	public class FreeLeftToRightMovement : PlayableCharacter
	{
		...
		public enum PossibleMovementAxis
		{
			X,
			Y,
			Z
		...
		[Tooltip("the axis you want your character to move on")]
		...
		[Tooltip("the movement inertia (the higher it is, the longer it takes for it to stop / change direction")]
		public float MovementInertia;

		/// if true, the character will stop before reaching the level's death bounds
		[Tooltip("if true, the character will stop before reaching the level's death bounds")]
		...
		protected override void Start()
		{
			...
			switch (MovementAxis)
			{
				case PossibleMovementAxis.X:
					_minBound = LevelManager.Instance.transform.position.x +
					            LevelManager.Instance.DeathBounds.center.x -
					            LevelManager.Instance.DeathBounds.extents.x +
					            _boundsSecurity;
					_maxBound = LevelManager.Instance.transform.position.x +
					            LevelManager.Instance.DeathBounds.center.x +
					            LevelManager.Instance.DeathBounds.extents.x -
					            _boundsSecurity;
				...
				case PossibleMovementAxis.Y:
					_minBound = LevelManager.Instance.transform.position.y +
					            LevelManager.Instance.DeathBounds.center.y -
					            LevelManager.Instance.DeathBounds.extents.y +
					            _boundsSecurity;
					_maxBound = LevelManager.Instance.transform.position.y +
					            LevelManager.Instance.DeathBounds.center.y +
					            LevelManager.Instance.DeathBounds.extents.y -
					            _boundsSecurity;
				...
				case PossibleMovementAxis.Z:
					_minBound = LevelManager.Instance.transform.position.z +
					            LevelManager.Instance.DeathBounds.center.z -
					            LevelManager.Instance.DeathBounds.extents.z +
					            _boundsSecurity;
					_maxBound = LevelManager.Instance.transform.position.z +
					            LevelManager.Instance.DeathBounds.center.z +
					            LevelManager.Instance.DeathBounds.extents.z -
					            _boundsSecurity;
