...
namespace MoreMountains.InfiniteRunnerEngine
{
	...
	public class PlayableCharacter : MonoBehaviour
	{
		...
		[Tooltip("should we use the default mecanim ?")]
		...
		[Tooltip("if true, the object will try to go back to its starting position")]
		public bool ShouldResetPosition = true;

		// the speed at which the object should try to go back to its starting position
		[Tooltip("the speed at which the object should try to go back to its starting position")]
		...
		[Tooltip("the distance tolerance at which a character is considered grounded")]
		public float GroundDistanceTolerance = 0.05f;

		/// the duration (in seconds) of invincibility on spawn
		[Tooltip("the duration (in seconds) of invincibility on spawn")]
		...
		protected virtual void ComputeDistanceToTheGround()
		{
			...
			if (_rigidbodyInterface.Is2D)
			{
				...
				RaycastHit2D raycastLeft = MMDebug.RayCast(_raycastLeftOrigin, Vector2.down,
					_distanceToTheGroundRaycastLength, 1 << LayerMask.NameToLayer("Ground"), Color.gray, true);
				if (raycastLeft)
				{
				...
				RaycastHit2D raycastRight = MMDebug.RayCast(_raycastRightOrigin, Vector2.down,
					_distanceToTheGroundRaycastLength, 1 << LayerMask.NameToLayer("Ground"), Color.gray, true);
				if (raycastRight)
				{
					if (raycastLeft)
			...
			{
				// we cast a ray to the bottom to check if we're above ground and determine the distance
				RaycastHit raycast3D = MMDebug.Raycast3D(transform.position, Vector3.down,
					_distanceToTheGroundRaycastLength, 1 << LayerMask.NameToLayer("Ground"), Color.green, true);
		...
		protected virtual void UpdateAllMecanimAnimators()
		{
			...
			MMAnimatorExtensions.UpdateAnimatorFloatIfExists(_animator, "VerticalSpeed",
				_rigidbodyInterface.Velocity.y);
		}
		...
		protected virtual void ResetPosition()
		{
			if (ShouldResetPosition)
			{
				if (_grounded)
				{
					_rigidbodyInterface.Velocity =
						new Vector3((_initialPosition.x - transform.position.x) * (ResetPositionSpeed),
							_rigidbodyInterface.Velocity.y, _rigidbodyInterface.Velocity.z);
				}
			}
		}
