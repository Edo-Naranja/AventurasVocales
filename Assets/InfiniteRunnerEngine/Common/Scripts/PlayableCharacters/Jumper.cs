...
namespace MoreMountains.InfiniteRunnerEngine
{
	public class Jumper : PlayableCharacter
	{
		...
		[Tooltip("the vertical force applied to the character when jumping")]
		public float JumpForce = 20f;

		/// the number of jumps allowed
		[Tooltip("the number of jumps allowed")]
		public int NumberOfJumpsAllowed = 2;

		/// the minimum time (in seconds) allowed between two consecutive jumps
		[Tooltip("the minimum time (in seconds) allowed between two consecutive jumps")]
		public float CooldownBetweenJumps = 0f;

		/// can the character jump only when grounded ?
		[Tooltip("can the character jump only when grounded ?")]
		...
		[Tooltip("the speed at which the character falls back down again when the jump button is released")]
		public float JumpReleaseSpeed = 50f;

		/// if this is set to false, the jump height won't depend on the jump button release speed
		[Tooltip("if this is set to false, the jump height won't depend on the jump button release speed")]
		...
		[Tooltip("the minimal time, in seconds, that needs to have passed for a new jump to be authorized")]
		...
		[Tooltip("a duration, after a jump, during which the character can't be considered grounded (to avoid jumps left to be reset too soon depending on context)")]
		...
		public virtual IEnumerator JumpSlow()
		{
			while (_rigidbodyInterface.Velocity.y > 0)
			{
				...
				_rigidbodyInterface.Velocity = new Vector3(_rigidbodyInterface.Velocity.x, newGravity.y,
					_rigidbodyInterface.Velocity.z);
				yield return 0;
			}
		}
	}
