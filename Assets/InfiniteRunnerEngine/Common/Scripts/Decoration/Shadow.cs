...
namespace MoreMountains.InfiniteRunnerEngine
{
	...
	public class Shadow : MonoBehaviour
	{
		...
		[Tooltip("The prefab to instantiate as the object's shadow")]
		public GameObject ShadowPrefab;

		/// The collision mask to use to place the shadow
		[Tooltip("The collision mask to use to place the shadow")]
		public LayerMask GroundMask = 0;

		/// The length of the raycast used to detect the ground
		[Tooltip("The length of the raycast used to detect the ground")]
		...
		[Tooltip("The maximum horizontal offset to apply to the shadow when far away")]
		...
		protected virtual void Update()
		{
			...
			if (_hit)
			{
				...
				float distance = _shadow.transform.position.y - transform.position.y + _boxCollider.bounds.size.y / 2 +
				                 _boxCollider.offset.y;
				_shadowOffsetBasedOnHeight =
					(ShadowMaxHorizontalDistance / (Mathf.Abs(MaximumVerticalDistance / distance))) * Vector3.right;
