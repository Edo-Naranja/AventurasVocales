...
namespace MoreMountains.InfiniteRunnerEngine
{
	...
	public class ScenarioManager : MonoBehaviour
	{
		...
		[MMInformation(
			"Here you can set the frequency (in seconds) at which the scenario will be evaluated. Depending on how many events are in your scenario, this can be heavy on performance, so you might want to space the evaluations more.",
			MoreMountains.Tools.MMInformationAttribute.InformationType.Info, false)]
		[Tooltip("The frequency at which the scenario will be evaluated, in seconds")]
		public float EvaluationFrequency = 1;

		[MMInformation("You can also choose to use the MMEventManager class to propagate events outside of this class.",
			MoreMountains.Tools.MMInformationAttribute.InformationType.Info, false)]
		[Tooltip("If you set this to true, the ScenarioManager will use the MMEventManager to propagate events.")]
		...
		protected virtual void AddTimeEvent(string timeInStringNotation, Action action, string eventName = "")
		{
			...
			_scenario.Add(new ScenarioEvent(0f, startTime, action, eventName,
				ScenarioEvent.ScenarioEventTypes.TimeBased));
		}
		...
		{
			_scenario.Add(new ScenarioEvent(startScore, 0f, action, eventName,
				ScenarioEvent.ScenarioEventTypes.ScoreBased));
		}
			...
			foreach (var item in _scenario)
			{
				if (item.ScenarioEventType == ScenarioEvent.ScenarioEventTypes.TimeBased)
				{
					...
					if (item.StartTime <= currentTime && item.Status == true)
					{
						...
						{
							MMGameEvent.Trigger(item.MMEventName);
						}
					}
				}
